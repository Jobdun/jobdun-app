...
void main() {
  ...
  testWidgets('password toggle reveals and hides the value', (tester) async {
    ...
    expect(find.byIcon(AppIcons.hidden), findsOneWidget);
    expect(find.byIcon(AppIcons.visible), findsNothing);

    await tester.tap(find.byIcon(AppIcons.hidden));
    await tester.pumpAndSettle();

    // After tap: eye icon visible (value now revealed).
    expect(find.byIcon(AppIcons.visible), findsOneWidget);
    expect(find.byIcon(AppIcons.hidden), findsNothing);
  });
