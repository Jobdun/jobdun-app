-- ============================================================
-- Jobdun schema reconciliation — CONSOLIDATED, IDEMPOTENT
-- Generated 2026-05-16 from migrations 20260516000001 + 20260516000002.
-- Safe to run via supabase db push OR pasted into the SQL editor,
-- repeatedly. Pre-launch, no data: additive only.
-- ============================================================
--
-- NOTE: ALTER TYPE ... ADD VALUE cannot run inside a transaction block in
-- Postgres (not even from within a DO/PL-pgSQL block wrapped in BEGIN/COMMIT).
-- The document_status enum-add is therefore hoisted here, ABOVE the BEGIN,
-- with its own idempotent DO/EXCEPTION guard. Everything else remains inside
-- the transaction for atomicity.
-- ============================================================

-- (hoisted — must run outside any transaction block)
DO $$ BEGIN
...
-- NOTE: document_status ADD VALUE 'expired' was hoisted above BEGIN (see top of file).
